...
<div id="cart-sidebar" class="cart-sidebar" aria-hidden="true" data-refresh-url="{$urls.pages.index}">
    ...
    <div class="cart-sidebar__panel">
        <div class="cart-sidebar__header">
            ...
            <button type="button" class="cart-sidebar__close" data-cart-sidebar-close
        ...
        <div class="cart-sidebar__body">
            ...
            <div class="cart-sidebar__products js-cart-sidebar-products"
                ...
                {if isset($cart) && $cart.products_count > 0}
                    <button type="button" class="btn btn-danger btn-sm cart-sidebar__empty-btn js-cart-empty-all"
                        title="{l s='Vider le panier' d='Shop.Theme.Checkout'}">
                ...
                {if isset($cart.products) && is_array($cart.products) && $cart.products|@count > 0}
                    {foreach from=$cart.products item=product}
                        ...
                        <div class="cart-sidebar__product" data-id-product="{$product.id_product}"
                            data-id-product-attribute="{$product.id_product_attribute|default:0}"
                            data-id-customization="{$product.id_customization|default:0}"
                            data-update-url="{$product.update_quantity_url|default:''}">
                            ...
                            <div class="cart-sidebar__product-details">
                                ...
                                <div class="cart-sidebar__product-qty">
                                    <button type="button" class="cart-sidebar__qty-btn js-cart-qty-down"
                                        aria-label="{l s='Decrease' d='Shop.Theme.Actions'}">−</button>
                                    <span class="cart-sidebar__qty-value">{$qty}</span>
                                    <button type="button" class="cart-sidebar__qty-btn js-cart-qty-up"
                                        aria-label="{l s='Increase' d='Shop.Theme.Actions'}">+</button>
                                </div>
                            </div>
                            <div class="cart-sidebar__product-actions">
                                <a href="{$product.remove_from_cart_url}" class="cart-sidebar__remove js-cart-remove"
                                    data-link-action="delete-from-cart" data-id-product="{$product.id_product}"
                                    data-id-product-attribute="{$product.id_product_attribute|default:0}"
                                    data-id-customization="{$product.id_customization|default:0}"
                                    data-product-url="{$product.url}"
                                    data-product-name="{$product.name|escape:'htmlall':'UTF-8'}" rel="nofollow"
                                    title="{l s='Remove' d='Shop.Theme.Checkout'}">
        ...
        <div class="cart-sidebar__footer js-cart-sidebar-footer"
            {if !isset($cart) || $cart.products_count == 0}style="display:none;" {/if}>
            <div class="cart-sidebar__total">
                <span class="cart-sidebar__total-label">{l s='Total' d='Shop.Theme.Checkout'}</span>
                <span
                    class="cart-sidebar__total-value js-cart-sidebar-total">{if isset($cart.totals.total.value)}{$cart.totals.total.value}{elseif isset($cart.totals.total.amount)}{$cart.totals.total.amount}{else}{/if}</span>
            </div>
